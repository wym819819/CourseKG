...
class CoTPrompt(Prompt):
    ...
    def get_ner_prompt(self, content: str) -> str:
        ...
        """
        prompt = {
            "instruction": "你是专门进行实体抽取的专家。请对input的内容进行总结根据总结从中抽取出符合schema类型的实体。最后请给出你的总结和抽取到的实体列表，返回的格式为 ```json\n[\"entity1\", \"entity2\"]\n```",
            "schema": entities,
            "examples": [{
                "input":
    ...
    def get_re_prompt(self, content: str, entities: list[str]) -> str:
        ...
        """
        prompt = {
            "instruction": "你是专门进行关系判别的专家，请对输入的实体列表根据已有文本片段判断两两之间的关系，如果两两之间无关系或关系不在所指定的关系范围relations中，则不返回。头尾实体不应该相同。返回的格式为 ```json\n[{\"head\": \"\", \"relation\": \"\", \"tail\": \"\"}]\n```",
            "relations": relations,
            "examples": {
    ...
    def get_ae_prompt(self, content: str, entities: list[str]) -> str:
        ...
        """
        prompt = {
            "instruction": "你是专门进行属性抽取的专家，请对输入的实体列表根据已有文本片段各自抽取他们的属性值。属性范围只能来源于提供的attributes，属性值无需完全重复原文，可以是你根据原文进行的总结，如果实体没有能够总结的属性值则不返回。返回格式为 ```json\n{\"entity1\": {\"attribute1\":\"value\"}}\n```",
    ...
    def get_best_attr(self, entity: str, attr: str, values: list[str]) -> str:
        ...
        """
        prompt = {
            "instruction": "你是专门进行属性判别的专家，请从实体的属性对应的值列表中选择一个最佳的值，返回其下标。下标从0开始。只需要返回一个数字即可。",
            "examples": [{
            ...
            "input": f"实体为: '{entity}', 属性为: '{attr}', 属性值列表为: {values}"
        }
        return json.dumps(prompt, indent=4, ensure_ascii=False)
